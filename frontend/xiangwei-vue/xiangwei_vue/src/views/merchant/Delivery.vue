    ...
    >
      <el-form :model="editForm" ref="editFormRef" label-width="80px">
        <el-form-item label="快递公司" prop="expressCompany" :rules="[{ required: true, message: '请输入快递公司', trigger: 'blur' }]">
          <el-input v-model="editForm.expressCompany" placeholder="请输入快递公司"></el-input>
        </el-form-item>
        <el-form-item label="快递单号" prop="expressNo" :rules="[{ required: true, message: '请输入快递单号', trigger: 'blur' }]">
          <el-input v-model="editForm.expressNo" placeholder="请输入快递单号"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
...
const editForm = reactive({
  id: null,
  orderId: null, // 虽然修改时不改这个，但为了完整性保留
  expressCompany: '',
  expressNo: ''
})
...
const fetchDeliveryList = async () => {
  ...
  try {
    ...
    // 只有当输入框有值时，才传递 orderId
    if (searchOrderId.value) {
      params.orderId = searchOrderId.value
    }

    const res = await request.get('/api/delivery/list', { params })
...
// 点击查询按钮
const handleSearch = () => {
  queryParams.pageNum = 1 // 查询时重置到第一页
  fetchDeliveryList()
}
...
const submitEdit = async () => {
  ...
  await editFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        const res = await request.post('/api/delivery/update', {
          id: editForm.id,
...
const getStatusText = (status) => {
  return { 1: '已发货', 2: '已签收', 3: '作废', 4: '退货' }[status] || '未知状态'
}
