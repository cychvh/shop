      ...
      >
        ...
        <el-table-column label="商品图" width="100">
          <template #default="scope">
            <el-image
              :src="getFullImgUrl(scope.row.imageurl)"
        ...
        <el-table-column label="小计" width="120">
          <template #default="scope">
            <span class="subtotal-text">￥{{ formatPrice(scope.row.price * scope.row.quantity) }}</span>
          </template>
        </el-table-column>
...
// 修改数量
const handleQuantityChange = async (newVal, row) => {
  try {
    await request.put('/api/cart/updateCart', { 
      id: row.id, 
      quantity: newVal 
    })
  } catch (err) {
...
const removeCartItem = async (cartId) => {
  try {
    const res = await request.delete('/api/cart/delCart', { params: { cartId } })
    if (res.code === 200 || res.code === '200') {
    ...
    try {
      ...
      const orderDTO = {
        ...
        orderItems: multipleSelection.value.map(item => ({
          productId: item.productid,  // 对应后端 @JsonProperty("productid") 或 productId
      ...
      const res = await request.post('/api/order/addOrder', orderDTO)
